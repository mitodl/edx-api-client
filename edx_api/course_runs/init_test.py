...
class CourseRunsTest(TestCase):
    ...
    def setUpClass(cls):
        with open(os.path.join(os.path.dirname(__file__),
                               'fixtures/course_run.json')) as file_obj:
            cls.course_run_json = json.loads(file_obj.read())

        with open(os.path.join(os.path.dirname(__file__),
                               'fixtures/course_run_list.json')) as file_obj:
        ...
        cls.course_run_clone_url = urljoin(base_edx_url, CourseRuns.course_run_clone_url)
    ...
    def test_clone_course_run(self, mock_req):
        ...
        response = self.course_run_client.clone_course_run(source_course_id="test", destination_course_id="test1")
        assert response == self.course_run_responses[1]['json']
    ...
    def test_create_course_run(self, mock_req):
        ...
        response = self.course_run_client.create_course_run(org="ORG", number="NUMBER", run="RUN", title="Test Course")
        ...
        with pytest.raises(ValueError):
            self.course_run_client.create_course_run(org="ORG", number="NUMBER", run="RUN", title="Test Course", start="2023-01-01T00:00:00Z")
        with pytest.raises(ValueError):
            self.course_run_client.create_course_run(org="ORG", number="NUMBER", run="RUN", title="Test Course", end="2023-01-01T00:00:00Z")
    ...
    def test_update_course_run(self, mock_req):
        ...
        response = self.course_run_client.update_course_run(course_id=course_id, title="Updated Title", pacing_type="self_paced")
        assert response.json == self.course_run_responses[0]['json']
        # Check that start and end are passed together otherwise none passed at all
        with pytest.raises(ValueError):
            self.course_run_client.update_course_run(course_id=course_id, title="Updated Title", start="2023-01-01T00:00:00Z")
        with pytest.raises(ValueError):
            self.course_run_client.update_course_run(course_id=course_id, title="Updated Title", end="2023-01-01T00:00:00Z")
